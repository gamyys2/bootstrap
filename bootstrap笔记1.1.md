...
* 实现混合器的声明，变量的声明模块化;
  ...
  > 当sass文件名称以 “_” 开头，是被用来引入到其他scss文件的，这样的文件不会单独被编译，
  >
  > 称为独立的模块，模块化的方式可更有利于scss文件的维护；
  >
  > 引入的方式  : @import "filename"   // 注意不需要加后缀名;
...
* bootstrap优点：
  ...
  * 有大量的组件样式，接受定制
* bootstrap缺点
  * 如果项目中有大量的非bootstrap“风格”的样式存在，需要很多css重写；
  * 会有兼容问题，对于低版本的ie浏览器;
...
* 栅格的基本原理
  ...
  * 手机:  xs    <768px                   自动
  * 平板:  sm   768px -- 992px     750px
  * 普屏:  md   992px -- 1200px   970px
  * 宽屏:   lg     >1200px                1170px
...
* box-sizing 盒模型的解析方式
  ...
  * content-box  内容区宽度（默认值）
  * border-box   内容区宽度+内填充+边框宽度 
...
* 栅格单位的组合使用
  ...
  * 为什么要组合使用 --- 实现不同屏幕下的不同尺寸

* 响应式列重置  --  解决栅格内容过多导致的“卡住”问题
...
* hbuilder 格式化代码快捷键 ctrl+shif+f


* 字体图标的使用 : Glyphicons + 图标类名
